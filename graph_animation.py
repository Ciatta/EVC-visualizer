...
loop=1
dir = []
def animate_node(n_frames, fig, ax, G, pos, G_guards, pos_g, pos_g1, labels, font_size):
    global loop, dir
    loop=1
    dir = []
    steps=[0]
    ...
    animation = FuncAnimation(fig, update, frames=range(n_frames+1), repeat=False, fargs=[steps, fig, ax, G, pos, G_guards, pos_g, pos_g1, labels, font_size], )
    return animation


def update(frame, steps, fig, ax, G, pos, G_guards, pos_g, pos_g1, labels, font_size):
    global loop, dir
